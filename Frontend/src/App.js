...
function App() {
  ...
  const handleEditTask = async (task_id, title, description) => {
    try {
      await axios.put(`api/updatetasks/${task_id}`, {
        title,
        description,
      });
      const updatedTasks = tasks.map((task) => {
  ...
  return (
    <div className="container">
      ...
      <h2 className="heading">All Tasks</h2>
      {message && (
        <p className={`alert ${message.includes('successfully') ? 'success' : 'error'}`}>{message}</p>
      )}
      <TaskList tasks={tasks} handleDeleteTask={handleDeleteTask} handleEditTask={handleEditTask} />
    </div>
  );
}
